...
namespace OpenTKImageViewer.UI
{
    class PixelValueShader
    {
        ...
        /// <returns></returns>
        public Vector4 GetPixelColor(int x, int y, int radius, int level)
        {
            // set variables
            shaderProgram.Bind();
        ...
        private static void SetTextureLevel(int level)
        {
            GL.Uniform1(2, level);
        }
        ...
        private static string GetComputeSource()
        {
            return "#version 430 core\n" +
                   ...
                   "layout(location = 2) uniform int lod;\n" +
                   "\n" +
                   "layout(std430, binding = 1) buffer pixelDstBuffer { vec4 pixelDst; };" +
                   "void main(){\n" +
                   "vec4 sum = vec4(0.0);\n" +
                   "ivec2 size = textureSize(src, lod);\n" +
                   ...
                   "                                    clamp(size.y - y - 1,0,size.y-1)), lod);\n" +

#setting title, Fast Approximate Anti-Aliasing
#setting description, Post-process anti-aliasing technique
#setting type, TEX2D

#param UseSrgb, use_srgb, bool, false

// use highest preset for image viewer purpose
#define FXAA_PRESET 5

// helper functions, since FXAA works on perceived color (post srgb conversion)
float4 toSrgb(float4 c){
	if(!use_srgb) return c;
    float3 r;
    [unroll]
    for(int i = 0; i < 3; ++i){
        if( c[i] >= 1.0) r[i] = 1.0;
...
float4 fromSrgb(float4 c){
    ...
    for(int i = 0; i < 3; ++i){
        if(c[i] >= 1.0)
            ...
            r[i] = pow(max((c[i] + 0.055)/1.055, 0.0), 2.4);
    }
    return float4(r, c.a);
}
...
float4 FxaaTexLod0( float2 pos) {
    return toSrgb(src_image.SampleLevel(linearSampler, pos.xy, 0.0));
}
/*--------------------------------------------------------------------------*/
float4 FxaaTexGrad( float2 pos, float2 grad) {
    return toSrgb(src_image.SampleGrad(linearSampler, pos.xy, grad, grad));
}
/*--------------------------------------------------------------------------*/
#define FxaaTexOff(pos, off, rcpFrame) toSrgb(src_image.SampleLevel(linearSampler, pos, 0.0, off))
...
----------------------------------------------------------------------------*/
    float3 rgbN = FxaaTexOff(pos.xy, FxaaInt2( 0,-1), rcpFrame).xyz;
    float3 rgbW = FxaaTexOff(pos.xy, FxaaInt2(-1, 0), rcpFrame).xyz;
    float3 rgbM = FxaaTexOff(pos.xy, FxaaInt2( 0, 0), rcpFrame).xyz;
    float3 rgbE = FxaaTexOff(pos.xy, FxaaInt2( 1, 0), rcpFrame).xyz;
    float3 rgbS = FxaaTexOff(pos.xy, FxaaInt2( 0, 1), rcpFrame).xyz;
...
----------------------------------------------------------------------------*/
    float3 rgbNW = FxaaTexOff(pos.xy, FxaaInt2(-1,-1), rcpFrame).xyz;
    float3 rgbNE = FxaaTexOff(pos.xy, FxaaInt2( 1,-1), rcpFrame).xyz;
    float3 rgbSW = FxaaTexOff(pos.xy, FxaaInt2(-1, 1), rcpFrame).xyz;
    float3 rgbSE = FxaaTexOff(pos.xy, FxaaInt2( 1, 1), rcpFrame).xyz;
...
----------------------------------------------------------------------------*/
    ...
    for(int i = 0; i < FXAA_SEARCH_STEPS; i++) {
        ...
        #else
            if(!doneN) lumaEndN = 
                FxaaLuma(FxaaTexGrad(posN.xy, offNP).xyz);
            if(!doneP) lumaEndP = 
                FxaaLuma(FxaaTexGrad(posP.xy, offNP).xyz);
        #endif
...
{
	float2 texC = (pixelCoord + 0.5) / float2(size);
	float4 srcPixel = src_image[pixelCoord];
	return fromSrgb(float4(FxaaPixelShader(texC, FxaaFloat2(1.0 / size.x, 1.0 / size.y)), srcPixel.a));
}